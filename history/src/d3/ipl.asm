...
entry:
  ...
  mov AX, 0x0820              ; 将程序装载入内存中0820:0000位置（绝对地址为8200）
  ...
  mov CL, 2                   ; 扇区 2
  mov AL, 63                  ; 读入扇区数
                              ; 若要不出错误，经尝试可知最多一次连续读取63个扇区。如果考虑上引导扇区
                              ; 的话便是64个
  mov BX, 0
  mov DL, 0x00                ; A驱动器:如果使用虚拟机练习，此处需要特别注意是以软盘镜像启动
                              ; 还是硬盘镜像启动。若是硬盘镜像则驱动器显然并非0号（第一软驱）
